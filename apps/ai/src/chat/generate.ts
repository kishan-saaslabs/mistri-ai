...
function buildChatPrompt(input: ChatGenerationInput): ChatMessage[] {
  const system = [
    ...
    "",
    "The Scope line below names the deal/call this evidence was drawn from — that name is an external CRM",
    "label, not something anyone says on the call, so it will legitimately NEVER appear inside the Evidence",
    "text itself. If the Question asks about that scope by name (\"tell me about X\", \"what is X\" where X is",
    "the deal/call named in Scope), that name IS the evidence below — answer from it and cite it, exactly",
    "as if the question had said \"tell me about this deal/call.\" Do not refuse just because the literal",
    "scope name doesn't appear in the evidence text — of course it doesn't, it's a label, not a transcript.",
    "",
